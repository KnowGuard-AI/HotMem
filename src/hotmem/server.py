...
def create_app(
    ...
    async def hydrate_one(memory_id: str, req: HydrateOneRequest | None = None):
        ...
        # If a profile is requested, return JSON.
        if req is not None:
            with Timer() as t:
                try:
                    ph = hydrate_with_profile(
                        db, memory_id, profile=req.profile, base_dir=base_dir, verify=req.verify
                    )
                except KeyError:
                    return JSONResponse(
                        status_code=404,
        ...
        # Default: raw bytes (backward-compatible).
        with Timer() as t:
            try:
    ...
    async def list_files(identifier: str | None = None):
        ...
        with Timer() as t:
            ...
            # Check existence (stat only, no bytes read)
    ...
    async def hydrate_batch(req: HydrateBatchRequest):
        ...
        with Timer() as t:
            ...
            for mid in req.memory_ids:
                try:
                    ph = hydrate_with_profile(
                        db, mid, profile=req.profile, base_dir=base_dir, verify=req.verify
                    )
