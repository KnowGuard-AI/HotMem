...
def test_regression_hydrate_response_unchanged(client: TestClient, tmp_path: Path):
    ...
    # plus the additive `invalid` count (#67, interchange contract §7) and the
    # additive embedding-disposition fields (#78).
...
def test_migration_adds_events_table(tmp_path: Path):
    ...
    try:
        ...
        assert db.count() == 0
        # user_version bumped to 3 (events), 4 (#73 sync checkpoints),
        # then 5 (#101 handoff ledger).
...
def test_migration_is_idempotent_for_events(tmp_path: Path):
    ...
    conn.commit()
    conn.close()
    # Opening again must not error and must keep user_version at 5.
    db = MemoryDB(db_path)
    try:
        version = db._conn.execute("PRAGMA user_version").fetchone()[0]
        assert version == 5
